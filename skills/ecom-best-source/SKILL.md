...
目标: 从 JD 或京东万商商品出发，找 1688 可采购同款货源，输出 Top 5 和结构化报告。不要依赖外部 `huoyuan`；它只是历史参考，执行时不得读取、导入或调用任何机器上的历史 `huoyuan` 目录。
...
- 输出必须包含完整 1688 详情链接、价格、起批数/库存判断、拒绝原因统计和 Top 5 推荐。
...
- `找货_<商品简写>_<YYYYMMDD>.json`: 完整 target、召回统计、final、rejected_reasons。
