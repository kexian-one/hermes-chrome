...
4. 候选召回完成后，优先运行 `scripts/sourcing_pipeline.py` 一次性完成 target 合成、最终筛选和 CSV 导出；不要让 LLM 手工拼大 JSON 或手工生成整份 CSV。
...
本 skill 的数据源私有密钥放在项目根 `config.yaml` 的 `ecom_best_source` 段。大模型统一使用项目根 `llm` 段，不在 `ecom_best_source` 下单独配置。检查配置状态时运行:
...
候选召回完成后，必须优先用流水线脚本直接生成最终 CSV，避免 LLM 读取大 JSON 后手工拼接 `merged_input.json` / `final.json` / CSV:

```bash
python skills/ecom-best-source/scripts/sourcing_pipeline.py --jd-product jd_product.json --candidates candidates.json --buy-multiple 3 --output "找货_商品简写_20260615.csv"
```

对应 `run_ecom_script`:

```json
{"script": "sourcing_pipeline.py", "args": ["--jd-product", "jd_product.json", "--candidates", "candidates.json", "--buy-multiple", "3", "--output", "找货_商品简写_20260615.csv"], "timeout_seconds": 60}
```
...
`sourcing_rules.py` 输出 `final`、`rejected`、`rejected_reasons` 和每个候选的 `score_breakdown`。如果不能运行脚本，必须人工照 `references/final_filter_rules.md` 同样执行，不得改权重。
...
- `找货_<商品简写>_<YYYYMMDD>.csv`: 给人看的 Top 6 表。

禁止用 `write_file` 写最终 JSON、`final.json`、`merged_input.json`、调试 JSON、Markdown 或文本报告到任务输出目录。需要中间 JSON 时，可以让脚本通过 `--output` 写临时文件，或用 `write_file` 写临时 JSON；系统会把这些临时文件放到隐藏 scratch 目录，不会作为产出上传。正常路径下最终 CSV 应由 `sourcing_pipeline.py --output ...csv` 生成，只有脚本不可用时才用 `write_file` 手写 CSV。
...
- 不写 Excel 公式。
- 详情链接必须是完整 URL。
- 拒绝/风险说明需要放在 CSV 列里，不另写 JSON 报告。
